...
public class RelationA implements WritableComparable<RelationA>{
	...
	public boolean isCondition(int col, String value, String mode) throws IOException{
		if (mode == "smaller"){
			if(col == 0 && Integer.parseInt(value) > this.id)
				return true;
			else if(col == 1 && name.compareTo(value) < 0)
				return true;
			...
			throw new IOException("The last argument must be \"bigger\", \"equal\" or \"smaller\".");
		}
	}
